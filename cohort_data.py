...
def unique_houses(filename):
    ...
    # Creating an empty list for houses
    house_name_list = []

    # Looping each line in the text file
    for data in filename:

        # Stripping the whitespaces to the right
        data = data.rstrip()

        # Split line by '|' to get a list of strings
        data = data.split('|')

        # Assigning data to indexes from the data list
        first_name = data[0]
        last_name = data[1]
        house_name = data[2]
        instructor = data[3]
        ...
        # Checking if there is a house name
        if house_name:
            house_name_list.append(data[2]) # Append the house to the empty list

    # Create a house set to get unique houses    
    Houses = set(house_name_list)
